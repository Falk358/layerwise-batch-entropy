...
def main():
   ...
   # eval_loader = torch.utils.data.DataLoader(ds_eval, **test_kwargs)
    train_loader, eval_loader, test_loader = dataloader.get_cifar10_loader(batch_size= args.batch_size, num_workers=7)

    model = FNN(args=args).to(device)
    criterion = CELoss() # use cross entropy loss for this experiment, since we use lbe to prune
